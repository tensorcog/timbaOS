#!/bin/bash
# Customer Validation Test Suite
set -euo pipefail
...
test_list_customers() {
    echo ""; echo "TEST: List Customers"
    res=$(api_request "GET" "/api/pos/customers" ""); st=$(echo "$res" | tail -n1); body=$(echo "$res" | head -n-1)
    [ "$st" -eq 200 ] && { cnt=$(echo "$body" | grep -o '"id":' | wc -l); pass "Retrieved customers (found $cnt)"; } || fail "Failed (status: $st)"
}

run_all_tests() {
    echo "╔════════════════════════════════════════╗"; echo "║  Customer Validation Test Suite       ║"; echo "╚════════════════════════════════════════╝"; echo ""
...
case "${1:-}" in test_list_customers) login "admin@billssupplies.com" "password"; $1 ;; "") run_all_tests ;; *) echo "Unknown test: $1"; exit 1 ;; esac
